...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($error)) {
        ...
        $query = "INSERT INTO borrowers (username, password, first_name, last_name, email, created_at, id) 
                  VALUES (?, ?, ?, ?, ?, NOW(), ?)";
        $stmt = $conn->prepare($query);

        if ($stmt) {
            $stmt->bind_param("sssss", $username, $hashedPassword, $first_name, $last_name, $email, $role_name);
